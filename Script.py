class script(object):
    START_TXT = """<b>ʜᴇʟʟᴏ {}✨
    
◈ I Aᴍ A Pᴏᴡᴇʀғᴜʟ 4GB Fɪʟᴇ Rᴇɴᴀᴍᴇʀ Bᴏᴛ.
◈ I Cᴀɴ Rᴇɴᴀᴍᴇ Fɪʟᴇs, Cʜᴀɴɢᴇ Tʜᴜᴍʙɴᴀɪʟs, Cᴏɴᴠᴇʀᴛ Bᴇᴛᴡᴇᴇɴ Vɪᴅᴇᴏ Aɴᴅ Fɪʟᴇ, Aɴᴅ Sᴜᴘᴘᴏʀᴛ Cᴜsᴛᴏᴍ Tʜᴜᴍʙɴᴀɪʟs Aɴᴅ Cᴀᴘᴛɪᴏɴs.\n
<blockquote>• Mᴀɪɴᴛᴀɪɴᴇʀ: <a href="https://t.me/missqueenbotx">@MɪꜱꜱǫᴜᴇᴇɴBᴏᴛx</a></blockquote></b>"""

    ABOUT_TXT = """<b>╭───────────⍟
├◈ ᴍy ɴᴀᴍᴇ : Rᴇɴᴀᴍᴇʀ 4GB V3
├◈ Oᴡɴᴇʀ: : <a href=https://t.me/sonali_sahaibot>Sᴏɴᴀʟɪ</a> 
├◈ Dᴇᴠᴇʟᴏᴩᴇʀꜱ : <a href=https://t.me/CoderLuffyTalkBot>ʟᴜғғʏ</a> 
├◈ Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ: <a href=https://t.me/missqueenbotx>𝙈𝙞𝙨𝙨𝙌𝙪𝙚𝙚𝙣𝘽𝙤𝙩 𝙭</a>   
├◈ Lɪʙʀᴀʀy : <a href=https://github.com/pyrogram>Pyʀᴏɢʀᴀᴍ</a>
├◈ Lᴀɴɢᴜᴀɢᴇ: <a href=https://www.python.org>Pʏᴛʜᴏɴ 𝟹</a>
├◈ Dᴀᴛᴀ Bᴀꜱᴇ: <a href=https://cloud.mongodb.com>Mᴏɴɢᴏ DB</a>
├◈ Bᴜɪʟᴅ Vᴇʀꜱɪᴏɴ: <a href=https://t.me/missqueenbotx>Rᴇɴᴀᴍᴇʀ V3</a> 
╰───────────────⍟</b>"""

    STATS = """ 
◽️ Uᴩᴛɪᴍᴇ: {}
◽️ CPU Uꜱᴀɢᴇ: {}%
◽️ RAM Uꜱᴀɢᴇ: {}%
◽️ Tᴏᴛᴀʟ Dɪꜱᴋ: {}
◽️ Uꜱᴇᴅ Dɪꜱᴋ: {} ({}%)
◽️ Fʀᴇᴇ Dɪꜱᴋ: {}"""

    STATUS = """
╭➦ Bot Uptime: {}
├❑ Disk Space: {}
├✎ Used Space: {}
├〄 Free Space: {}
├▲ Upload: {}
├▼ Download: {}
├❖ Cpu: {}
├❀ Ram: {}
╰➥ Disk: {}"""
    
    HELP_TXT = """
ʜᴇʏ {} ʜᴇʀᴇ ɪs ᴛʜᴇ ʜᴇʟᴘ ғᴏʀ ᴍʏ ᴄᴏᴍᴍᴀɴᴅs."""
    
    THUMB_TXT = """<b><blockquote>How To Set Thumbnail</blockquote>

<blockquote>Please send any photo to me, and I will save it automatically.</blockquote>

❍ /delthumb : Use this command to delete old thumbnail.

❍ /viewthumb : Use this command to view your current thumbnail.</b>"""

    PREMIUM_TXT = """<b>ɴᴀᴍᴇ:- <code>{}</code>
ᴜꜱᴇʀ ɪᴅ:- <code>{}</code>

<blockquote><b>ᴛʜᴀɴᴋ ʏᴏᴜ ꜰᴏʀ ᴄᴏɴꜱɪᴅᴇʀɪɴɢ ᴏᴜʀ ᴘʀᴇᴍɪᴜᴍ ᴘʟᴀɴ.</b></blockquote>

ᴄᴜʀʀᴇɴᴛ ᴘʟᴀɴ: {}

 ☞ ᴅᴀɪʟʏ ᴜᴘʟᴏᴀᴅ: {}
 ☞ ᴘᴀʀᴀʟʟᴇʟ ᴘʀᴏᴄᴇꜱꜱ: 2
 ☞ ᴛɪᴍᴇᴏᴜᴛ: 5 ᴍɪɴᴜᴛᴇꜱ
 ☞ ᴛɪᴍᴇ ɢᴀᴘ: ʏᴇꜱ</b>"""

    FREE_PLAN = """<b><u>💗 ꜰʀᴇᴇ ᴘʟᴀɴ ᴜꜱᴇʀ</u>
    
💰 <u>ᴘʟᴀɴ ᴘʀɪᴄᴇ</u>: 0

➻ ᴅᴀɪʟʏ ᴜᴘʟᴏᴀᴅ ʟɪᴍɪᴛ: 5ɢʙ
☞ ᴠᴀʟɪᴅɪᴛʏ: ʟɪғᴇᴛɪᴍᴇ

☞ ᴜᴘʟᴏᴀᴅ 2ɢʙ ꜰɪʟᴇꜱ
☞ ᴅᴀɪʟʏ ᴜᴘʟᴏᴀᴅ: 5ɢʙ
☞ ᴘᴀʀᴀʟʟᴇʟ ᴘʀᴏᴄᴇꜱꜱ: 2
☞ ᴛɪᴍᴇᴏᴜᴛ: 5 ᴍɪɴᴜᴛᴇꜱ
☞ ᴛɪᴍᴇ ɢᴀᴘ: ʏᴇꜱ</b>"""

    SILVER_TXT = """<b>🥈 <u>sɪʟᴠᴇʀ ᴘʟᴀɴ</u>

☞ ᴜᴘʟᴏᴀᴅ 4ɢʙ ꜰɪʟᴇꜱ: ᴛʀᴜᴇ
☞ ᴅᴀɪʟʏ ᴜᴘʟᴏᴀᴅ: 100.0ɢʙ
☞ ᴘᴀʀᴀʟʟᴇʟ ᴘʀᴏᴄᴇꜱꜱ: 2
☞ ᴛɪᴍᴇ ɢᴀᴘ: ɴᴏ ᴛɪᴍᴇ ɢᴀᴘ

💰 <u>ᴘʟᴀɴ ᴘʀɪᴄᴇ</u>

☞ ᴘʀɪᴄᴇ: ₹59
☞ ᴠᴀʟɪᴅɪᴛʏ: 𝟹𝟶ᴅᴀʏs</b>"""

    GOLDEN_TXT = """<b>🎖️ <u>ɢᴏʟᴅ ᴘʟᴀɴ</u>

☞ ᴜᴘʟᴏᴀᴅ 4ɢʙ ꜰɪʟᴇꜱ: ᴛʀᴜᴇ
☞ ᴅᴀɪʟʏ ᴜᴘʟᴏᴀᴅ: 100.0ɢʙ 
☞ ᴘᴀʀᴀʟʟᴇʟ ᴘʀᴏᴄᴇꜱꜱ: 3
☞ ᴛɪᴍᴇ ɢᴀᴘ: ɴᴏ ᴛɪᴍᴇ ɢᴀᴘ

💰 <u>ᴘʟᴀɴ ᴘʀɪᴄᴇ</u>

☞ ᴘʀɪᴄᴇ: ₹119
☞ ᴠᴀʟɪᴅɪᴛʏ: 6𝟶ᴅᴀʏs</b>"""

    DIAMOND_TXT = """<b>💎 <u>ᴅɪᴀᴍᴏɴᴅ ᴘʟᴀɴ</u>

☞ ᴜᴘʟᴏᴀᴅ 4ɢʙ ꜰɪʟᴇꜱ
☞ ᴅᴀɪʟʏ ᴜᴘʟᴏᴀᴅ: 100.0ɢʙ
☞ ᴘᴀʀᴀʟʟᴇʟ ᴘʀᴏᴄᴇꜱꜱ: 5
☞ ᴛɪᴍᴇ ɢᴀᴘ: ɴᴏ ᴛɪᴍᴇ ɢᴀᴘ

💰 <u>ᴘʟᴀɴ ᴘʀɪᴄᴇ</u>

☞ ᴘʀɪᴄᴇ: ₹179
☞ ᴠᴀʟɪᴅɪᴛʏ: 9𝟶ᴅᴀʏs</b>"""

    UPGRADE_PLAN_TXT = """**Fʀᴇᴇ Pʟᴀɴ Usᴇʀs**
Dᴀɪʟʏ Uᴘʟᴏᴀᴅ ʟɪᴍɪᴛ 𝟻GB Rᴇɴᴀᴍᴇ Oɴʟʏ Uᴘᴛᴏ 𝟸ɢʙ Fɪʟᴇs
Pʀɪᴄᴇ 𝟶
 
**🥈 Sɪʟᴠᴇʀ Pʟᴀɴ 🥈**
Dᴀɪʟʏ  Uᴘʟᴏᴀᴅ  ʟɪᴍɪᴛ : 10𝟶GB
Pʀɪᴄᴇ Rs 59
Vᴀʟɪᴅɪᴛʏ: 𝟹𝟶ᴅᴀʏs
 
**🥇Gᴏʟᴅ Pʟᴀɴ🥇**
Dᴀɪʟʏ Uᴘʟᴏᴀᴅ ʟɪᴍɪᴛ : 10𝟶GB
Pʀɪᴄᴇ Rs.119
Vᴀʟɪᴅɪᴛʏ: 6𝟶ᴅᴀʏs

**💎  Dɪᴀᴍᴏɴᴅ Pʟᴀɴ 💎**
Dᴀɪʟʏ Uᴘʟᴏᴀᴅ ʟɪᴍɪᴛ : 𝟷𝟶𝟶GB
Pʀɪᴄᴇ Rs.179
Vᴀʟɪᴅɪᴛʏ: 9𝟶ᴅᴀʏs
 
# <blockquote>**ᴜᴘɪ ɪᴅ ➩**:- <code>sonalisaha1564@okhdfcbank</code></blockquote>
 
**Aғᴛᴇʀ Pᴀʏᴍᴇɴᴛ Sᴇɴᴅ Sᴄʀᴇᴇɴsʜᴏᴛs Tᴏ Aᴅᴍɪɴ**"""
    
    CAPTION_TXT = """<b><blockquote><u>How To Set A Custom Caption</u></blockquote>

EX- <code>/set_caption {filename}
{filesize}
{duration}</code>

❍ /set_caption - Set a custom caption for your content.

❍ /see_caption - To view your custom caption.

❍ /delete_caption - Remove your custom caption from my database.</b>"""
    
    SOURCE_TXT = """<b>ɴᴏᴛᴇ :

⚠️ᴛʜɪꜱ ʙᴏᴛ ɪꜱ ᴀɴ ᴘʀɪᴠᴀᴛᴇ ꜱᴏᴜʀᴄᴇ ᴘʀᴏᴊᴇᴄᴛ

<blockquote>⇒ ɪ ᴡɪʟʟ ᴄʀᴇᴀᴛᴇ ᴀ ʙᴏᴛ ꜰᴏʀ ʏᴏᴜ
⇒ ᴄᴏɴᴛᴀᴄᴛ ᴍᴇ - [♚ ᴀᴅᴍɪɴ ♚](https://t.me/sonali_sahaibot)</blockquote></b>"""
