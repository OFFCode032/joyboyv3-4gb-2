...
def insert(chat_id):
            user_id = int(chat_id)
            user_det = {"_id":user_id,"file_id":None ,"caption":None ,"daily":0 ,"date":0 , "uploadlimit" :5368709120,"used_limit":0,"usertype":"Free","prexdate" : None, "referpoint" : 0, "trailclaim":"False"}
            try:
...
def refer(chat_id,point):
    dbcol.update_one({"_id":chat_id},{"$set":{"referpoint":point}})

def claim(chat_id, claim):
    dbcol.update_one({"_id":chat_id},{"$set":{"trailclaim":claim}})
